...
6. Para desarrollo, configurar `VITE_API_PROXY_TARGET=http://127.0.0.1:3001`, `VITE_BASE_PATH=/` y ejecutar `npm run dev`. Para alojarla en la raíz del dominio, construir con `VITE_BASE_PATH=/` y `VITE_GASTROBAR_MENU_URL=https://sierradorada.co/gastrobar/menu` en el entorno de `npm run build`. Las variables Vite se fijan al compilar; no contienen secretos.
...
El Dockerfile del frontend acepta `--build-arg VITE_BASE_PATH=/ --build-arg VITE_GASTROBAR_MENU_URL=https://sierradorada.co/gastrobar/menu`.
...
La dirección preparada es **https://sierradorada.co/gastrobar/menu**. Se tomó del dominio que ya figura en el proyecto; no se realizó un despliegue de producción durante esta tarea. Publicar y verificar la dirección desde un celular antes de imprimir las tarjetas definitivas. Si el dominio cambia en el futuro, conservarlo con redirección permanente para no reemplazar los QR de las mesas. Conservar también la ruta cuando se cambie de servidor.
